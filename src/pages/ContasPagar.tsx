...
export default function ContasPagar() {
  ...
  const parcelaIdsPagar = useMemo(() => contas.map(c => (c as any).parcela_id), [contas]);
  const { rateioMap: rateioMapPagar } = useCentroCustoRateio(parcelaIdsPagar);

  const contasEnriquecidas = useMemo(() => {
    if (rateioMapPagar.size === 0) return contas;
    return contas.map(c => {
      const pid = (c as any).parcela_id;
      if (pid && rateioMapPagar.has(pid)) {
        return { ...c, centros_custo_rateio: rateioMapPagar.get(pid) };
      }
      return c;
    });
  }, [contas, rateioMapPagar]);
  ...
  const fetchContas = async () => {
    try {
      ...
      const contasComServicos = await Promise.all(contasFiltradasPorContrato.map(async (item: any) => {
    ...
    if (dateRange?.from && dateRange?.to) {
      ...
      if (dateFilterType === 'vencimento') {
        ...
        if (conta.data_pagamento) {
          dateToCheck = new Date(conta.data_pagamento + 'T00:00:00');
        } else {
          return false;
        }
      }
              ...
              {paginatedContas.map(conta => <TableRow key={conta.id}>
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="font-medium text-sm" title={conta.fornecedores?.nome_fantasia || conta.fornecedores?.razao_social || '-'}>
                  ...
                  <TableCell className="font-semibold text-destructive text-sm">
                    {formatCurrency(conta.valor_parcela)}
                  </TableCell>
                  <TableCell>
                    <Badge variant={getStatusVariant(conta.status_pagamento, conta.data_vencimento)} className="text-xs">
