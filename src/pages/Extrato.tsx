...
export default function Extrato() {
  ...
  const handleSaveEdit = async (data: EditParcelaData) => {
    ...
    try {
      ...
      // Detecta se o usuário realmente alterou o bruto, o líquido ou os ajustes.
      // Se nada disso mudou (ex.: só alterou data/conta/descrição), preservamos
      // o valor atual para não sobrescrever o líquido com o bruto.
      ...
      const liquidoAtual = Number(selectedLancamento.valor ?? 0);
      const liquidoNovo = Number(data.valor_total ?? 0);
    ...
    try {
      if (batchActionType === 'change-date' && data?.newDate) {
        ...
        for (const lanc of selectedLancamentos) {
          ...
          if (allPaid) {
            const result = await revertBaixa(lanc.id, lanc.origem as 'pagar' | 'receber');
            if (result.blocked) {
              blockedCount++;
              if (!firstBlockedMsg) firstBlockedMsg = result.blockedMessage || '';
            }
          } else {
        ...
        if (blockedCount > 0) {
          toast({
            title: `${blockedCount} reversão(ões) bloqueada(s)`,
            description: firstBlockedMsg,
            variant: "destructive",
          });
        } else {
          toast({
