...
export default function PortalReembolsos() {
  const { user } = useAuth();
  const [open, setOpen] = useState(false);

  const { data = [] } = useQuery({
    queryKey: ['portal-list', user?.id, 'reembolso'],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Receipt className="h-6 w-6" /> Reembolsos</h1>
          <p className="text-muted-foreground">Solicite reembolsos de despesas com upload do comprovante.</p>
        </div>
        <Button onClick={() => setOpen(true)}><Plus className="h-4 w-4 mr-1" /> Novo reembolso</Button>
      </div>
      ...
      <NovaSolicitacaoDialog open={open} onOpenChange={setOpen} tipo="reembolso" />
    </div>
  );
}
