...
export function ProtectedRoute({ children, portal = false }: { children: React.ReactNode; portal?: boolean }) {
  const { user, loading, signOut } = useAuth();
  const { role, loading: roleLoading, permissions, isPortalOnly } = useUserRole();
  ...
  // Portal-only roles must stay in /portal
  if (isPortalOnly && !location.pathname.startsWith('/portal')) {
    return <Navigate to="/portal" replace />;
  }

  // Non-portal users trying to enter portal area redirect to home
  if (!isPortalOnly && portal && !permissions.canAccessPortal) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground">
            <p>Por favor, aguarde o administrador atribuir as permissões necessárias à sua conta.</p>
          </div>
          <div className="pt-4">
